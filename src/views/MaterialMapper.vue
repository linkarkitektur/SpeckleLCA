...
export default {
  ...
  methods: {
    ...
    loadJSONtemplate() {
      var templateJson = require("@/assets/LCA_byg/merged_rest.json");
      return templateJson;
    },
    ...
    generateBuildingJSON() {
      var buildingJSON = require("@/assets/LCA_byg/dummy_building.json");
    ...
    getMaterialQuantity(category, isMultiPart, combinedUnits) {
      ...
      if (isMultiPart) {
        ...
        !isMultiPart &&
        (combinedUnits.includes("m3") || combinedUnits.includes("m"))
      ) {
    ...
    addCategory() {
      if (this.className && this.quantity) {
        ...
        this.currentCategoryMapper[this.className] = {
          staticFullName: "",
          isTemporary: true,
        };
        this.currentCategoryMapper[this.className + "#" + this.className] = {
          staticFullName: "",
          isTemporary: true,
        };
    ...
    async getAccessToken() {
      ...
      try {
        const response = await axios.post(
          "https://oneclicklcaapp.com/app/api/login",
          body
        );
        if (response.status === 200) {
        ...
        data.forEach((e2) => {
          if (e2.CLASS.split("#")[0] === e1) {
            ...
            obj.sub_categories.push({
              ...
              resourceSubType:
                this.selectedMapper.data[`${e2.CLASS}`].resourceSubType,
            });
          }
        });
