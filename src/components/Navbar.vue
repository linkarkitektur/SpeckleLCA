<template>
  <div id="nav" class="h-16">
    <Disclosure as="nav" class="fixed bg-white shadow w-full z-50" v-slot="{ open }">
            ...
            <Menu as="div" class="relative ml-3">
              <div>
                <MenuButton class="relative flex rounded-full bg-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                  ...
                  <img class="h-8 w-8 rounded-full" :src="speckleStore.user?.avatar" alt="" />
                </MenuButton>
              </div>
        ...
        <div class="border-t border-gray-200 pb-3 pt-4">
          <div class="flex items-center px-4">
            <div class="flex-shrink-0">
              <!-- Avatar image-->
              <img class="h-10 w-10 rounded-full" :src="speckleStore.user?.avatar" alt="" />
            </div>
            <div class="ml-3">
              <div class="text-sm font-medium text-gray-500"><span>{{speckleStore.user? speckleStore.user.name : "No User" }}</span></div>
            </div>
          </div>
