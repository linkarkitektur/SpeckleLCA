<template>
  <div>
    ...
    <p class="mt-1 styled-text">Set building codes to include, this will be used to segment results on building codes. Mostly used for certifications.</p>
...
const dropdownName = computed(() => 
  calculationSettings.value?.buildingCode?.key || selectedBuildingCode.value.key
)
...
const initializeGroupedBuildingCodes = () => {
  ...
  const data = calculationSettings.value.buildingCode?.data || selectedBuildingCode.value.data
  groupedBuildingCodes.value = JSON.parse(JSON.stringify(data)).map((item: BuildingCodeItem) => ({
    ...item,
...
const checkGroupSelection = (group: BuildingCodeItem, child: BuildingCodeItem, newVal: boolean) => {
  child.selected = newVal
...
watch(() => calculationSettings.value.buildingCode, (newVal) => {
  initializeGroupedBuildingCodes()
})
</script>