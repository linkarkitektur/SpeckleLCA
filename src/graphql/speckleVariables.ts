...
export function speckleSelection(sourceApplication: string) {
  ...
  if(sourceApplication.includes("Revit"))
    selection.push(...revitParameters);

  if(sourceApplication.includes("IFC"))
    selection.push(...ifcParameters);

  if(sourceApplication.includes("Rhino") && sourceApplication.includes("Grasshopper"))
