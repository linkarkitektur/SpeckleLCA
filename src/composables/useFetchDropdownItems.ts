...
export function useFetchDropdownItems() {
  ...
  const dropdownName = computed(() => {
    switch(navStore.activePage) {
      case 'Filtering': return 'Filter Groups'
      case 'Mapping': return 'Available Mappings'
      case 'Results': return 'Available Results'
      case 'Benchmark': return 'Available Benchmarks'
