...
export class EmissionCalculator {
	...
	calculateEmissions(): Results | boolean {
		...
		for (const geo of this.geo) {
			...
			const emissions: Emission = {
				[this.settingsStore.calculationSettings.standardImpactCategory]:
					{} as LifeCycleStageEmission
			}
	...
	private processAssembly(
		...
		for (const product of Object.values(products)) {
			...
			tempGeo.quantity.m3 =
				tempGeo.quantity.m2 * (parseFloat(product.metaData.thickness) / 1000)
	...
	private processEPD(
		...
		if (material.emission) {
			if (this.calculateMaterialEmissions(material, emissions, geo)) return true
			else return false
		}
		return false
	}
	...
	private calculateMaterialEmissions(
		...
		if (!product.emission[impactCategory]) {
			if (!product.emission['gwp']) return false
		...
		const materialFraction = product.materialFraction
			? product.materialFraction / 100
			: 1
		...
		const replacements = product.lifespan
			? Math.max(0, Math.ceil(projectLifespan / product.lifespan) - 1)
			: 0
		...
		for (const stage in matEmission) {
			...
			if (
				this.settingsStore.calculationSettings.includedStages.relevantStages.some(
				...
				if (value !== null && !isNaN(Number(value))) {
					const quantity = product.unit.includes('pcs')
						? 1
						: geo.quantity[product.unit]
					const emissionValue =
						parseFloat(value as string) * quantity * materialFraction
		...
		if (
			replacements > 0 &&
			this.settingsStore.calculationSettings.replaceB4WithProductionStages
		) {
			// Determine B4 emission based on setting
			const b4Emission =
				(emissions[impactCategory]['a1a3'] || 0) +
	 ...
	 */
	private addEmissionsToGeo(emissions: Emission, geo: GeometryObject): Results {
