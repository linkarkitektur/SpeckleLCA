...
export const useFirebaseStore = defineStore('firebase', {
	...
	actions: {
		...
		async fetchLatestFilters(projectId: string): Promise<FilterLog[] | null> {
			...
			try {
				...
				if (!querySnapshot.empty) {
					...
					const filterStates = querySnapshot.docs.map((doc) =>
						doc.data()
					) as FilterLog[]
					return filterStates
				} else {
		 ...
		 */
		async fetchAssemblyList(
			projectId: string | boolean
		): Promise<AssemblyList | null> {
			this.loading = true
			this.error = null
			try {
				let q
				if (projectId === true) {
					q = query(collection(db, 'projectAssemblies'))
				} else {
					q = query(
		 ...
		 */
		async fetchCalculationSettings(
			projectId: string
		): Promise<CalculationSettingsLog[] | null> {
			this.loading = true
			this.error = null
			...
			try {
				...
				if (!specificSnapshot.empty || !genericSnapshot.empty) {
					const combinedSettings: CalculationSettingsLog[] = [
						...specificSnapshot.docs,
						...genericSnapshot.docs
					].map((doc) => ({
						...doc.data()
