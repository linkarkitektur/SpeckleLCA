...
function ArcProgressChart(data: ChartData[], options: ChartOptions = {}) {
  ...
  function drawChart(svgEl: SVGSVGElement, tooltipEl: HTMLDivElement, containerEl: HTMLDivElement) {
    ...
    const { mouseover, mousemove, mouseleave } = createMouseEventHandlers(tooltipDiv, containerEl)
    ...
    const centerY = margin.top + outerRadius + ((h - margin.top - margin.bottom) / 0.9)
    const arcGroup = graph.append('g')
      arcGroup.attr('transform', `translate(${centerX}, ${centerY}) scale(${scaleFactor})`)
    ...
    const foregroundArc = arcGroup.append('path')
