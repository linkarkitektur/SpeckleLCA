...
const zeroOffsets = computed(() => {
  ...
  return props.data.map(group => {
    ...
    const positiveHeight = (positiveSum / total) * scale * availableHeight.value
    const maxPositiveHeight = (maxPositive / total) * scale * availableHeight.value
...
const drawAxes = (data: NestedChartData[]) => {
  if (axesSvg.value && container.value) {
    ...
    const chart = d3.select(axesSvg.value)
      .append('g')
      .attr('transform', `translate(${margin.left}, 0)`)

    // Calculate max values
    const sumPositive = d3.max(data, group => 
...
const drawBarGroup = (svg: SVGSVGElement, groupData: NestedChartData) => {
  ...
  const positiveSeriesData = positiveData.map(d => {
    const item = { ...d, start: positiveOffset, end: positiveOffset + Math.abs(d.value) }
  ...
  const yScale = d3.scaleLinear().domain([0, negativeOffset]).range([0, height])
