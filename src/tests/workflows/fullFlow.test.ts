...
import { createTestPinia, setupSpeckleStore, setupFilterRegistry, setupMaterialStore, setupMapping } from '../setup/testUtils'
...
import { ResultCalculator, resultItemToChartData, resultItemToNestedChartData } from '@/utils/resultUtils'
...
describe('Full Workflow Test', () => {
  ...
  it('should load project and convert objects correctly', async () => {
    ...
    const wall = projectStore.currProject.geometry.find(obj => obj.id === 'wall-1')
    ...
    const concreteElements = projectStore.currProject.geometry
    ...
    expect(Array.from(otherMaterials).sort()).toEqual(['EPDM', 'Glass', 'Steel', 'Wood'])
  })
  ...
  it('should process objects through filtering workflow', async () => {
    ...
    expect(wallGroup.objects.reduce((sum, el) => sum + (el.quantity.m2 || 0), 0)).toBe(55) // 25 + 30
    expect(wallGroup.objects.reduce((sum, el) => sum + (el.quantity.m3 || 0), 0)).toBe(16.5) // 7.5 + 9.0

    // Verify quantities for floor slabs
    const floorGroup = tree.children.find(g => g.name === 'Floors')
    expect(floorGroup.objects.reduce((sum, el) => sum + (el.quantity.m2 || 0), 0)).toBe(220) // 100 + 120
    expect(floorGroup.objects.reduce((sum, el) => sum + (el.quantity.m3 || 0), 0)).toBe(44) // 20 + 24

    // Verify material distribution
    const materialGroups = new Map<string, number>()
    tree.children.flatMap(g => g.objects).forEach(el => {
    ...
    const totalArea = tree.children.flatMap(g => g.objects)
      .reduce((sum, el) => sum + (el.quantity.m2 || 0), 0)
    const totalVolume = tree.children.flatMap(g => g.objects)
  ...
  it('should execute complete workflow from load to material mapping', async () => {
    ...
    const columnEmissions = columnGroup.objects.map(el => el.results?.[el.results.length - 1])
    ...
    const allStructuralElements = tree.children.flatMap(g => g.objects)
      .filter(el => ['Walls', 'Columns', 'Floors', 'Beams'].includes(el.simpleParameters.category))
  ...
  it('should convert results to chart data formats', async () => {
    ...
    const wallElements = tree.children
      .find(g => g.name === 'Walls').objects
  ...
  it('should export results to JSON format', async () => {
    ...
    const stageResults = jsonData.find(item => item.parameter === 'lifeCycleStages')
    ...
    const nestedResults = jsonData.find(item => item.parameter === 'parameters.category -> material.name')
    const nestedWalls = nestedResults.data.find(item => item.parameter === 'Walls')
    expect(nestedWalls.nested).toBeDefined()
    expect(Array.isArray(nestedWalls.nested)).toBe(true)
    const concreteData = nestedWalls.nested.find(m => m.parameter === 'Ready-mix concrete')
