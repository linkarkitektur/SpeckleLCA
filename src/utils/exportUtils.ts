...
export class ExportManager {
  ...
  public exportData(): void {
    ...
    switch (this.config.format) {
      ...
      case ExportFormat.Excel: {
        ...
        this.downloadFile(exportedContent, 'export.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        break
      }
      default:
  ...
  private exportToCSV(): string {
    ...
    const cleanedResultList: ResultList = this.cleanData(this.resultList).filter(item => item.data)
    const rows: string[] = []
    ...
    const collectKeysAndDepth = (group: GroupedResults, depth: number) => {
      ...
      if (group.quantity && typeof group.quantity === 'object') {
        Object.keys(group.quantity).forEach(key => quantityKeys.add(sanitize(key)))
      }
      // Add headers for emissions
      if (group.data?.emission?.[impactCategory]) {
        Object.keys(group.data.emission[impactCategory]).forEach(key => emissionKeys.add(sanitize(key)))
      }
