 ...
 */
export function geometryToLCSChartData(
	objects: GeometryObject[],
	impactCategory: string = 'gwp',
	resultKey: number = Number.MIN_SAFE_INTEGER
): ChartData[] {
	...
	for (const obj of objects) {
		...
		for (const lifeCycleStage in result.emission[impactCategory]) {
			...
			groupedData.set(
				lifeCycleStage,
				currentValue + result.emission[impactCategory][lifeCycleStage]
			)
		}
	}
 ...
 */
export function resultItemToNestedChartData(
	...
	for (const groupedResult of resultItem.data) {
		...
		if (!groupedResult.nested || groupedResult.nested.length === 0) {
			const topLevelArray: ChartData[] = Array.from(
				topLevelData,
				([label, { value, ids }]) => ({
					label: label,
					value: roundNumber(value, 2),
 ...
 */
function groupedResultToChartData(
	groupedResult: GroupedResults,
	groupedData: Map<string, ChartData>
): void {
 ...
 */
function groupedResultToLCSChartData(
	groupedResult: GroupedResults,
	groupedData: Map<string, ChartData>
): void {
	...
	for (const lifeCycleStage in groupedResult.data.emission[impactCategory]) {
		...
		const lifecycleStage =
			groupedResult.data.emission[impactCategory][lifeCycleStage]
		...
		else materialData.value += lifecycleStage / area
 ...
 */
export function geometryToChartData(
	objects: GeometryObject[],
	chartParameter: string,
	LCSData: boolean = false,
	newResList: boolean = false
): ChartData[] {
	...
	const resultItem = resCalc.resultList.find(
		(item) => item.parameter === chartParameter
	)

	// If we are calculating for LCS, we need to convert the data differently
	if (LCSData) return resultItemToLCSChartData(resultItem)
 ...
 */
export function geometryToNestedChartData(
	objects: GeometryObject[],
	chartParameter: string
): NestedChartData[] {
	// Create result aggregator and calculator
 ...
 */
export function emissionToNumber(
	emission: Emission,
	perArea: boolean = true
): number {
	const settingsStore = useSettingsStore()
 ...
 */
export function getResultLogEmissions(
	resultLog: ResultsLog,
	parameter: string
): Emission {
	const resItem = resultLog.resultList.find(
		(item: ResultItem) => item.parameter === parameter
	)
...
export function extractEmissionsFromResultItem(
	resultItem: ResultItem
): Emission[] {
		...
		for (const impactCategory in source) {
			...
			for (const stage in source[impactCategory]) {
				result[impactCategory][stage] =
					(result[impactCategory][stage] || 0) +
					(source[impactCategory][stage] || 0)
			}
		}
	}
	mergeIntoResult(a)
...
export function addQuantity(a: Quantity, b: Quantity): Quantity {
	...
	const allKeys = new Set<MetricUnits>([
		...(Object.keys(a || {}) as MetricUnits[]),
		...(Object.keys(b || {}) as MetricUnits[])
	])
...
export class ResultCalculator {
	...
	public aggregate(save: Boolean = true, calcResultList = true): void {
		...
		if (save) this.saveResults()
	}

	private aggregateLifeCycleStagesResult(
		geo: GeometryObject,
		emission: Emission
	): void {
		const settingsStore = useSettingsStore()
		...
		for (const stage in emission[impactCategory]) {
			let stageGroup = this.lifeCycleStagesResult.data.find(
				(group) => group.parameter === stage
			)
			...
			stageGroup.quantity = addQuantity(
				stageGroup.quantity,
				geo.quantity || { m: 0 }
			)
	...
	private aggregateTotalEmissions(emission: Emission): void {
		for (const impactCategory in emission) {
			...
			for (const stage in emission[impactCategory]) {
				...
				this.totalEmission[impactCategory][stage] =
					currentTotal + emissionAmount
			}
		}
	}
	...
	private aggregateEmissionsForResultList(geo: GeometryObject): void {
		for (const index in this.resultList) {
			...
			if (paramValue) {
				...
				let groupedResult = paramItem.data.find(
					(result) => result.parameter === paramValue
				)
	...
	private interpolateEmissionFactor(energyType: EnergyType): number {
		...
		const valueDiff =
			upperBound.factors[energyType] - lowerBound.factors[energyType]
	...
	private calculateEnergyEmissions(): void {
		...
		const b6Group = this.lifeCycleStagesResult.data.find(
			(group) => group.parameter === 'b6'
		)
		if (!b6Group) {
	 ...
	 */
	private aggregateLifeCycleStages(groupedResult: GroupedResults): void {
		...
		for (const impactCategoryKey in aggregatedEmission) {
			const categoryEmission: LifeCycleStageEmission =
				aggregatedEmission[impactCategoryKey]
			...
			for (const stageKey in categoryEmission) {
				...
				stageGrouped.data.emission[impactCategoryKey][stageKey] =
					(stageGrouped.data.emission[impactCategoryKey][stageKey] ?? 0) +
					stageValue
